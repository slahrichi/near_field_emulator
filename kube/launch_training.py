...
def run(conf):
    ...
    template = load_file(conf.kube.train_job['paths']['template'])
    tag = conf.kube.train_job['paths']['template'].split("/")[-1]
    folder = conf.kube.train_job['paths']['template'].replace("/%s" % tag, "")
