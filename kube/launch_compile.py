...
def run(params):
    ...
    template = load_file(params['kube']['compile_job']['paths']['template'])
    
    tag = params['kube']['compile_job']['paths']['template'].split("/")[-1]
    folder = params['kube']['compile_job']['paths']['template'].replace("/%s" % tag, "")
    environment = Environment(loader = FileSystemLoader(folder))
    template = environment.get_template(tag)

    job_name = "%s" % (params['kube']['compile_job']['kill_tag'])

    template_info = {'job_name' : job_name,
                     'num_cpus' : str(params['kube']['compile_job']['num_cpus']),
                     'num_mem_lim' : str(params['kube']['compile_job']['num_mem_lim']),
                     'num_mem_req' : str(params['kube']['compile_job']['num_mem_req']),
                     'pvc_preprocessed' : params['kube']['pvc_preprocessed'],
                     'preprocessed_path' : params['kube']['compile_job']['paths']['data']['preprocessed_data'],
